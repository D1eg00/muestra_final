...
function draw() {
  ...
  // --- SELECCIÓN AUTOMÁTICA DE DIBUJO ---
 // --- SELECCIÓN AUTOMÁTICA DE DIBUJO ---
 // --- 5. SELECCIÓN AUTOMÁTICA DE DIBUJO ---
  ...
  if (i === 0) {
    ...
    dibujos[0].dibujar(buffer, vibActual, hug, saturacion, brillo, opC1); 
  } 
 else if (i === 1) { 
    // Obra 2: Curvas_2
    // Pasamos el 6to parámetro: 'intensidad'
    dibujos[1].dibujar(buffer, 100, vibActual, vibActualTiempo, saturacion, intensidad); 
  }
 else if (i === 2) {
    // Definimos la opacidad que reacciona al sonido (de 20 a 100)
    let opSonido = map(intensidad, 0.0, 1.0, 80, 20, true);
    
    // 100 es fijo para el fondo, opSonido es dinámico para las inestables de arriba
    dibujos[2].dibujar(buffer, 100, opSonido, strWC4, vibActual, saturacion); 
  }
  //background(0);
  guia.push();
  guia.colorMode(RGB);
